...
export default {
  ...
  cellphone(phone, password) {
    return fly.get(`/login/cellphone?phone=${phone}&password=${password}&cookie=xxx`)
  },
  // 邮箱登录
  ...
  // 官方
  official() {
    return fly.get(`/top/playlist?cat=官方&limit=51`)
  },
  // 精品
  essence() {
  ...
  // 民谣
  ballad() {
    return fly.get(`/top/playlist?cat=民谣&limit=51`)
  },
  // 所有榜单
  toplist() {
  ...
  // 搜索详情
  search(keywords, type, limit) {
    return fly.get(`/search?keywords=${keywords}&type=${type}&limit=${limit}`)
  },
  // 歌手
